...
if(work_experience_null!='Yes'):
    ...
    for i in range(0,number_of_works-2):
        c=0    
        for k in range(0,len(month)-1):
            if month[k] in Work_Period[i][0] and month[k] in Work_Period[i+1][1]:
                c=1
                print("Yes the works are continuous")                                   
        if c==0:
    ...
    for i in range(0,top):
        ...
        top_skills.append(n)
    for j in top_skills:
            if j.lower() in skills and j not in Technical:
                Technical.append(j)
    ...
    while(com.lower()!="done"):
        ...
        for i in questions.keys():
            if category in i and questions[i]!=[]:
                l=list(questions[i])
                qc=i
        print("Enter the number of questions not more than",len(l),'\n' )
        num=int(input())
        rq=(list((random.sample(l,num))))
        print(qc,' : ',rq)
        pair={qc:rq}
        rqd.update(pair)
